...
//устанавливливаем состояния для расчета положения стрелок, функции установки состояния передаем в props
...
//находим максимальную высоту для использования в качестве референса, передаем в props вместе с data

    const maxSum = Math.max(sumDev, sumTest, sumProd, (data?.norm ? data.norm : 0) );

//считаем координаты начал и концов стрелок

    const arrowStartY = 364 - devHeight;
    const arrowEndY = 364 - testHeight;
    const arrowEndY2 = 364 - prodHeight;

//считаем разницу в овалах на стрелках
...
//определяем цвета фону для овалов на стрелках 
...
//определяем положение стрелки в зависимости от фона - перевернута или ее отсутствие
